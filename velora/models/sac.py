...
class SACActor(NCPModule):
    ...
    distribution over actions. Usable with continuous action spaces.
    """
...
class LiquidSAC(RLAgent):
    """
    A Liquid variant of the Soft Actor-Critic (SAC) algorithm from the paper:
    [Soft Actor-Critic: Off-Policy Maximum Entropy Deep Reinforcement Learning with a Stochastic Actor](https://arxiv.org/abs/1801.01290).

    Includes enhances mentioned in the [Soft Actor-Critic Algorithms and Applications](https://arxiv.org/abs/1812.05905) paper.
    ...
    def _train_step(
        ...
        Returns:
            critic_loss (torch.Tensor): the total critic loss.
            actor_loss (torch.Tensor): the actor loss.
            entropy_loss (torch.Tensor): the entropy loss.
        """
        ...
        return critic_loss.detach(), actor_loss.detach(), entropy_loss.detach()
        ...
        with TrainHandler(
            ...
            for current_ep in range(1, n_episodes + 1):
                ...
                for current_step in range(1, max_steps + 1):
                    ...
                    critic_loss, actor_loss, entropy_loss = self._train_step(
                        batch_size, gamma
                    )
                    self._update_target_networks(tau)

                    handler.metrics.add_step(critic_loss, actor_loss, entropy_loss)
