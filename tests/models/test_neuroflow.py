...
class TestActorCriticModules:
    ...
    def test_actor_module_init(
        self, actor_module: ActorModule, actor_params: Dict[str, Any]
    ):
    ...
    def test_critic_module_init(
        self, critic_module: CriticModule, critic_params: Dict[str, Any]
    ):
    ...
    def test_entropy_module_init(
        self, entropy_module: EntropyModule, entropy_params: Dict[str, Any]
    ):
    ...
    def test_actor_predict(self, actor_params: Dict[str, Any]):
        # Create the actor module directly in the test to avoid JIT issues
        with patch("torch.jit.script") as mock_script:
            # Make mock_script return the original module to avoid JIT compilation
    ...
    def test_actor_forward(self, actor_params: Dict[str, Any]):
        # Create the actor module directly in the test to avoid JIT issues
        with patch("torch.jit.script") as mock_script:
            # Make mock_script return the original module to avoid JIT compilation
            mock_script.side_effect = lambda module: module
    ...
    def test_critic_predict(self, critic_params: Dict[str, Any]):
        # Create the critic module directly in the test to avoid JIT issues
        with patch("torch.jit.script") as mock_script:
            # Make mock_script return the original module to avoid JIT compilation
            mock_script.side_effect = lambda module: module
    ...
    def test_critic_target_predict(self, critic_params: Dict[str, Any]):
        # Create the critic module directly in the test to avoid JIT issues
        with patch("torch.jit.script") as mock_script:
            # Make mock_script return the original module to avoid JIT compilation
            mock_script.side_effect = lambda module: module
    ...
    def test_critic_update_targets(self, critic_module: CriticModule):
    ...
    def test_entropy_alpha(self, entropy_module: EntropyModule):
        # Test alpha property
        alpha = entropy_module.alpha
        assert isinstance(alpha, torch.Tensor)
        assert alpha.item() > 0  # Alpha should be positive

    def test_entropy_compute_loss(self, entropy_module: EntropyModule):
    ...
    def test_gradient_steps(
        ...
        # Test actor gradient step
        loss = torch.tensor(1.0, requires_grad=True)
        actor_module.gradient_step(loss)

        # Test critic gradient step
